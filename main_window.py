...
def index():
    ...
    if 'username' not in session:
        ...
        return render_template('index.html', games=games, title='@Hydragamestore')
    ...
    return render_template('index.html', username=session['username'], games=games, title='@Hydragamestore')
...
def buy_pur():
    ...
    for item in items:
        data = {
            "currency": "RUB",
            "amount": str(Game.query.filter_by(game_id=item.game_id).first().price) + "00"
        }
